...
try {
  ...
  const boomerangInputUrl = 'https://s1.boomerangserver.co.in/webhook/google-maps-scraper';
  const boomerangStatUrl  = 'https://s1.boomerangserver.co.in/webhook/google-maps-scraper-stats';
  ...
  // 4. FREE TRIAL CHECK
  // ──────────────────────────────
  const FREE_TRIAL_PLACES = 50;

  const trialStore  = await Actor.openKeyValueStore('boomerang-free-trials-google-maps');
  ...
  if (isFirstTime) {
    freeLeadsRemaining = FREE_TRIAL_PLACES;
    console.log(`\n🎁 First-time user detected! ${FREE_TRIAL_PLACES} free places applied.`);
  } else {
  ...
  const chargeableRows = Math.max(0, totalPlaces - freeLeadsRemaining);
  ...
  console.log('Free places    :', isFirstTime ? FREE_TRIAL_PLACES : 0);
  console.log('Chargeable     :', chargeableRows);
  ...
  while (batchJobs && batchJobs.length > 0) {
          ...
          await fetch(
            'https://frontend.boomerangserver.co.in/webhook/Status_and_output_universal',
            {
              ...
              body   : JSON.stringify({
                ...
                requestStatus     : 'Error',
                driveInputLink,
                boomerangOutputUrl: `https://s1.boomerangserver.co.in/webhook/google-maps-scraper-output?request_id=${request_id}`,
                batch_number,
                request_unique_id,
                batchFolderId,
                service_option_1  : serviceOption1,
    ...
    for (const result of batchStatusResults) {
      ...
      if (result.status !== 'Completed') {
        ...
        allOutputLinks.push('');
        console.log(`  ⚠️ Batch ${batch_number} (failed) — skipped dataset push.`);
        continue;
      }
      ...
      if (rowsPushed > 0) {
        ...
        if (freeForBatch > 0) {
          console.log(`  🎁 Batch ${batch_number} — ${freeForBatch} places FREE (trial). ${chargeableLeads} chargeable.`);
        }

        if (chargeableLeads > 0) {
          const batchCost = parseFloat((chargeableLeads * 0.005).toFixed(3));
          totalCharged   += batchCost;
          console.log(`  💳 Batch ${batch_number} — Charging for ${chargeableLeads} places ($${batchCost}). Total charged: $${totalCharged.toFixed(3)}`);
          try {
            await Actor.charge({ eventName: serviceOption1, count: chargeableLeads });
          } catch (chargeErr) {
            const remainingPlaces = totalPlaces - totalRowsDelivered;
            const remainingCost   = parseFloat((remainingPlaces * 0.005).toFixed(3));
            console.log(`\n❌ Insufficient Apify credits — run stopped.`);
            console.log(`✅ Places delivered : ${totalRowsDelivered}`);
            console.log(`💳 Total charged    : $${totalCharged.toFixed(3)}`);
            console.log(`⏳ Remaining places : ${remainingPlaces} (needs $${remainingCost} more)`);
            console.log(`👉 Add funds at apify.com/billing and re-run to get remaining places.`);
  ...
  console.log('════════════════════════════════════');
  console.log('Run ID           :', runId);
  console.log('Free places used :', totalFreeUsed);
